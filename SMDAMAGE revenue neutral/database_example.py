...
def add_agriculture_bids_to_sets(bidder_name, bids, AllBidPeriods, PT_set, APT_set, Bapt, Uapt, StartYear, scenario, PeriodsPerYear):
	"""
	Add agriculture bid information to the required sets and dictionaries.

	Args:
		bidder_name (str): Name of the bidder (e.g., 'Agriculture')
	...
	for t in AllBidPeriods:
		...
		for bidstep, (price, quantity) in enumerate(bids[:5]):  # Limit for demo
			APT_set.add((bidstep, bidder_name, t))
			Bapt[bidstep, bidder_name, t] = -price * scenario.discount_rate(t - StartYear)
			Uapt[bidstep, bidder_name, t] = quantity / PeriodsPerYear
...
def read_bids_from_database(scenario):
	...
	add_agriculture_bids_to_sets('Agriculture', agriculture_bids, AllBidPeriods, PT_set, APT_set, Bapt, Uapt, StartYear, scenario, PeriodsPerYear)
