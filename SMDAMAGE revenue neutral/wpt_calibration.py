...
def write_fitted_Wpt_to_csv(): # Write the fitted Wpt values to a CSV file in rows of pollutant and columns of years. Useful for analysis in Excel.
	with open(smdamage_main.getOutputDirectory() + "SMDAMAGE_fitted_Wpt.pkl", "rb") as mypickle: Wpt_dict = pickle.load(mypickle)
	...
	with open(smdamage_main.getOutputDirectory() + "SMDAMAGE_fitted_Wpt.csv", 'w', newline='', encoding='utf-8') as csv_file:
...
def run_SMDAMAGE_fit_W(scenario):
	...
	Pulse = smdamage_main.getPulse() # Reads the Pulse input file. Includes 'luc'.
	...
	Wpt_dict = {(p, float(t0)): 0.0 for p in Emitters + Removers for t0 in range(smdamage_main.getPulseDataLength())} # Time subscripts are floats because periods could be more often than years, e.g., 2025.0, 2025.5, ...
	scaleCelsius = 1000.0 # Thousandths of a degree.

	for p in ['C2F6', 'CF4', 'CH4', 'HFC125', 'HFC134a', 'HFC143a', 'N2O', 'SF6']:
		for t0 in range(smdamage_main.getPulseDataLength()): Wpt_dict[(p, float(t0))] = Pulse[p][1 + t0]*scaleCelsius/Pulse[p][0]

	for t0 in range(smdamage_main.getPulseDataLength()):  # Divide by Pulse[p][0] for Hector greenhouse gasses to normalize the pulse size.
	...
	Treetypes = smdamage_main.getTreeTypes() #['Loblolly_pine_150', 'Ponderosa_pine_150', 'Black_walnut_150', 'Loblolly_pine_10', 'Ponderosa_pine_10', 'Black_walnut_10', 'Loblolly_pine_24', 'Ponderosa_pine_103', 'Black_walnut_55']
	Treetype_carbon_removal = smdamage_main.get_Treetype_carbon_removal(Treetypes)
	...
	for tree in Treetypes: # Tree is planted in year 0. Tree sequesters TonsSequesteredPerPeriod tonnes/hectare in each sequesterperiod from 0 to 155.
		for treegrowthyear in range(0, 156): # Length of tree contract.
			for coolingyear in range(treegrowthyear, smdamage_main.getPulseDataLength()): # Growth in the last year of the contract has future cooling effects.
	...
	my_old_vpt = smdamage_main.open_pkl("SMDAMAGE " + smdamage_main.experimentTag_to_file_name(scenario)) # amount of activity p in year t from solution of model P1 or P2.
	...
	for p in ['C2F6', 'CF4', 'CH4', 'HFC125', 'HFC134a', 'HFC143a', 'N2O', 'SF6']:
		for t in smdamage_main.getBidPeriods(): Constant_PT_set.add((p,t))
	# if scenario.is_removal_luc:
	# 	for t in smdamage_main.getBidPeriods(): Constant_PT_set.add(('Carbon',t))
	# else: luc is not part of the model.

	Variable_PT_set = set() # But note that "luc" is not an activity in SMDAMAGE.
	# if scenario.is_removal_luc: Variable_w_activities = ['luc'] + ['Agriculture', 'Seaweed'] + smdamage_main.getTreeTypes()
	if scenario.is_removal_luc: Variable_w_activities = ['Carbon'] + ['luc'] + ['Agriculture', 'Seaweed'] + smdamage_main.getTreeTypes()
	else: Variable_w_activities = ['Carbon'] + ['Agriculture', 'Seaweed'] + smdamage_main.getTreeTypes()

	for p in Variable_w_activities:
		for t in smdamage_main.getBidPeriods(): Variable_PT_set.add((p,t))
		
	# Decision variables ------------------------------------------------------------------------------------------
	wpt = {}

	for t in range(smdamage_main.getPulseDataLength()): 
	...
	over_error_t = {t: LpVariable("over_error_t(" + str(t) + ")", 0.0, None) for t in smdamage_main.getModelPeriods()} # error below HectorTempt in year t.
	under_error_t = {t: LpVariable("under_error_t(" + str(t) + ")", 0.0, None) for t in smdamage_main.getModelPeriods()} # error above HectorTempt in year t.
	SMDAMAGE_temp_t = {t: LpVariable("SMDAMAGE_temp_t(" + str(t) + ")", None, None) for t in smdamage_main.getModelPeriods()} # Implied SMDAMAGE temperature.
	
	W_over_error_t = {t: LpVariable("over_error_t(" + str(t) + ")", 0.0, None) for t in range(smdamage_main.getPulseDataLength())} # error below HectorTempt in year t.
	W_under_error_t = {t: LpVariable("under_error_t(" + str(t) + ")", 0.0, None) for t in range(smdamage_main.getPulseDataLength())} # error above HectorTempt in year t.
	...
	SMDAMAGE_fit_W += lpSum(over_error_t[t] + under_error_t[t] for t in smdamage_main.getModelPeriods()), "Total error"
	# SMDAMAGE_fit_W += lpSum(over_error_t[t] + under_error_t[t] for t in smdamage_main.getModelPeriods()) + 100000.0*lpSum(W_over_error_t[t] + W_under_error_t[t] for t in range(smdamage_main.getPulseDataLength())), "Total error"
	
	# Constraints: temperature error. --------------------
	HectorTemp = smdamage_main.get_Hector_temperature(scenario) # the temperature in year t from Hector simulating solution Vp,t.
	
	# Constraints: warming  --------------------
	for t in smdamage_main.getModelPeriods():
	...
	# for u in range(smdamage_main.getPulseDataLength()): # Both ffi and luc pulses peak before 20 years after emission.
	...
	for u in range(smdamage_main.getPulseDataLength()): 
	...
	for tree in Treetypes: # If tree is planted in year 0, it sequesters TonsSequesteredPerPeriod tonnes/hectare in each sequesterperiod from 0 to 155.
		...
		for coolingyear in range(0, smdamage_main.getPulseDataLength()): # Growth in the last year of the contract has future cooling effects.
	...
	for u in range(20, smdamage_main.getPulseDataLength() - 1): # Both ffi and luc pulses peak before 20 years after emission.
	...
	# SMDAMAGE_fit_W.writeLP(smdamage_main.getOutputDirectory() + "Calibrate W " + smdamage_main.getExperimentTag() + ".lpt")
	...
	plotting_utils.plot_W_Hector_and_fitted(scenario, "Carbon", [Wpt_dict[('Carbon', float(t))] for t in range(smdamage_main.getPulseDataLength())], [wpt[('Carbon', float(t))].varValue for t in range(smdamage_main.getPulseDataLength())], smdamage_main.getPulseDataLength, smdamage_main.getOutputDirectory, smdamage_main.experimentTag_to_file_name)
	if scenario.is_removal_luc: plotting_utils.plot_W_Hector_and_fitted(scenario, "luc", [Wpt_dict[('luc', float(t))] for t in range(smdamage_main.getPulseDataLength())], [wpt[('luc', float(t))].varValue for t in range(smdamage_main.getPulseDataLength())], smdamage_main.getPulseDataLength, smdamage_main.getOutputDirectory, smdamage_main.experimentTag_to_file_name)
	# else: plotting_utils.plot_W_Hector_and_fitted(scenario, "Carbon", [Wpt_dict[('Carbon', float(t))] for t in range(smdamage_main.getPulseDataLength())], [wpt[('Carbon', float(t))].varValue for t in range(smdamage_main.getPulseDataLength())], smdamage_main.getPulseDataLength, smdamage_main.getOutputDirectory, smdamage_main.experimentTag_to_file_name)
	plotting_utils.plot_temps_Hector_and_fitted(scenario, [HectorTemp[t] for t in smdamage_main.getModelPeriods() if t <= 2300], [SMDAMAGE_temp_t[t].varValue for t in smdamage_main.getModelPeriods() if t <= 2300], [t for t in smdamage_main.getModelPeriods() if t <= 2300], smdamage_main.getOutputDirectory, smdamage_main.experimentTag_to_file_name)

	for p in Variable_w_activities:
		for t in range(smdamage_main.getPulseDataLength()): Wpt_dict[(p,float(t))] = wpt[(p, float(t))].varValue 
	# yourdictionary_to_CSV(Wpt_dict, "Wpt_dict " + smdamage_main.experimentTag_to_file_name(scenario))
	with open(smdamage_main.getOutputDirectory() + "SMDAMAGE_fitted_Wpt.pkl", "wb") as mypickle: pickle.dump(Wpt_dict, mypickle)
