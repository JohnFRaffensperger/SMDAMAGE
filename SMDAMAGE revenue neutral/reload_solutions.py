...
def init_db():
    ...
    c = conn.cursor()
    c.execute("""CREATE TABLE scenarios (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, discount_rate REAL, initial_temp REAL, tau REAL, is_revenue_neutral INTEGER, is_removal_luc INTEGER, use_updated_Wpt INTEGER, solver_status TEXT, total_revenue REAL, objective_value REAL, solution_datetime TEXT)""")
...
def reload_solutions():
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            c.execute("""INSERT INTO scenarios (name, discount_rate, initial_temp, tau, is_revenue_neutral, is_removal_luc, use_updated_Wpt, solver_status, total_revenue, objective_value, solution_datetime) VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
