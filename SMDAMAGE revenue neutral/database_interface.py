...
def get_calibrated_initial_temp(scenario):
	"""Return calibrated_initial_temp for the scenario, looked up by scenario.calibration_scenario_id."""
	import defaults_and_utilities # Late import to avoid circular dependency.
	db_path = defaults_and_utilities.getSolutionsDBPath()
	if scenario.calibration_scenario_id is None: raise ValueError("scenario.calibration_scenario_id is not set. Run run_SMDAMAGE_fit_W and set calibration_scenario_id first.")
	conn = sqlite3.connect(db_path)
	cursor = conn.cursor()
	cursor.execute("SELECT calibrated_initial_temp FROM scenarios WHERE id = ?", (scenario.calibration_scenario_id,))
	row = cursor.fetchone()
	conn.close()
	if row is None or row[0] is None: raise ValueError(f"No calibrated_initial_temp found for calibration_scenario_id={scenario.calibration_scenario_id}. Run run_SMDAMAGE_fit_W first.")
	return row[0]
