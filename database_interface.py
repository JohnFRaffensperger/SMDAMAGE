...
DB_NAME = "smdamage_data.db" # Create this database by running create_database.py first.
...
def do_query(sql, params=None):
    ...
    # if not os.path.exists(db_path): raise FileNotFoundError(f"Database '{db_path}' not found. Run create_database.py first.")
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    if params: cursor.execute(sql, params)
    else: cursor.execute(sql)
...
def get_bidders(): return do_query("SELECT * FROM bidders ORDER BY bidder collate nocase")
# print(get_bidders())
